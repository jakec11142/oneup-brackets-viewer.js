...
"use strict";
eval("{\n\nObject.defineProperty(exports, \"__esModule\", ({\n  value: true\n}));\nexports.updateFormCreator = exports.stageFormCreator = void 0;\nconst lang_1 = __webpack_require__(/*! ./lang */ \"./src/lang.ts\");\nconst brackets_manager_1 = __webpack_require__(/*! brackets-manager */ \"./node_modules/brackets-manager/dist/index.js\");\nconst stageTypes = ['single_elimination', 'double_elimination', 'round_robin'];\nconst roundRobinMode = ['simple', 'double'];\nconst roundRobinSeeds = ['groups.effort_balanced', 'groups.seed_optimized', 'groups.bracket_optimized'];\nconst eliminationOrderings = ['natural', 'reverse', 'half_shift', 'reverse_half_shift', 'pair_flip', 'inner_outer'];\nconst grandFinalTypes = ['none', 'simple', 'double'];\n/**\n * Creates a DOM form to create different stages for the brackets-manager\n *\n * @param configuration HTML element IDs to render this form to\n * @param submitCallable Callback function - what should happen onClick on the forms submit button?\n */\nfunction stageFormCreator(configuration, submitCallable) {\n  const parent = document.getElementById(configuration.parent_id);\n  if (null === parent) throw new DOMException('parent with ID: ' + configuration.parent_id + ' was not found!');\n  createBaseMask(parent, configuration);\n  const stageSelector = document.getElementById(configuration.html_stage_type_selector_id);\n  if (null === stageSelector) throw new DOMException('somehow we could not create a select!');\n  stageSelector.onchange = () => {\n    removeMaskFields(parent);\n    let stage;\n    switch (stageSelector.value) {\n      case 'round_robin':\n        stage = 'round_robin';\n        break;\n      case 'double_elimination':\n        stage = 'double_elimination';\n        break;\n      case 'single_elimination':\n        stage = 'single_elimination';\n        break;\n      default:\n        throw new DOMException('stage ' + stageSelector.value + ' seems to be not implemented yet.');\n    }\n    createMaskFields(configuration, stage, parent, submitCallable);\n  };\n  // We're creating a round_robin here by default cause its selected on first load.\n  createMaskFields(configuration, 'single_elimination', parent, submitCallable);\n}\nexports.stageFormCreator = stageFormCreator;\n/**\n * Creates a DOM form to update the current stage.\n *\n * @param configuration HTML element IDs to render this form to\n * @param changeCallable Callback function - what should happen onClick on the forms submit button?\n */\nfunction updateFormCreator(configuration, changeCallable) {\n  var _a, _b;\n  const urlParams = new URLSearchParams(window.location.search);\n  const id = (_a = urlParams.get('id')) !== null && _a !== void 0 ? _a : 0;\n  // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment\n  const storedBrackets = JSON.parse((_b = localStorage.getItem('brackets')) !== null && _b !== void 0 ? _b : '{}');\n  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access\n  const currentBracket = storedBrackets[id];\n  const parent = document.getElementById(configuration.parent_id);\n  if (null === parent) throw new DOMException('parent with ID: ' + configuration.parent_id + ' was not found!');\n  createBaseMask(parent, configuration, true, false);\n  const stageSelector = document.getElementById(configuration.html_stage_type_selector_id);\n  if (null === stageSelector) throw new DOMException('somehow we could not create a select!');\n  stageSelector.onchange = () => {\n    removeMaskFields(parent);\n    const stage = stageSelector.value || 'single_elimination';\n    createMaskFields(configuration, stage, parent, changeCallable, 'Edit');\n  };\n  const teamCountInput = document.getElementById(configuration.html_team_count_input_id);\n  teamCountInput.oninput = () => {\n    const stage = stageSelector.value || 'single_elimination';\n    applyForm(configuration, stage, changeCallable);\n  };\n  createMaskFields(configuration, currentBracket.stage[0].type, parent, changeCallable, 'Edit');\n}\nexports.updateFormCreator = updateFormCreator;\n/**\n * Removes all fields of the mask which are not the name and the stage selector.\n *\n * @param parent The HTML parent to hold the elements.\n */\nfunction removeMaskFields(parent) {\n  [...parent.children].filter(c => !c.classList.contains('base-input')).forEach(c => c.remove());\n}\n/**\n * @param parent HTMLElement\n * @param configuration FormConfiguration\n * @param setDefaultValues Whether to set default values for the form\n * @param showTeamNamesInput Whether to show the input for team names\n */\nfunction createBaseMask(parent, configuration, setDefaultValues = false, showTeamNamesInput = true) {\n  var _a, _b;\n  const urlParams = new URLSearchParams(window.location.search);\n  const id = (_a = urlParams.get('id')) !== null && _a !== void 0 ? _a : 0;\n  // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment\n  const storedBrackets = JSON.parse((_b = localStorage.getItem('brackets')) !== null && _b !== void 0 ? _b : '{}');\n  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access\n  const currentBracket = storedBrackets[id];\n  // Name\n  createInput(parent, 'text', configuration.html_name_id, (0, lang_1.t)('form-creator.stage-name-label'), (0, lang_1.t)('form-creator.stage-name-placeholder'), setDefaultValues ? currentBracket.stage[0].name : undefined, undefined, 1, 'base-input');\n  // Team names\n  if (showTeamNamesInput) {\n    createTextarea(parent, configuration.html_team_names_input_id, (0, lang_1.t)('form-creator.team-label'), (0, lang_1.t)('form-creator.team-label-placeholder'), undefined, 'base-input');\n  }\n  // or Team count\n  createInput(parent, 'number', configuration.html_team_count_input_id, showTeamNamesInput ? (0, lang_1.t)('form-creator.team-count') : 'Team count', (0, lang_1.t)('form-creator.team-count-placeholder'), setDefaultValues ? currentBracket.participant.length.toString() : '', '1', undefined, 'base-input');\n  // Stage selector\n  createSelect(parent, configuration.html_stage_type_selector_id, (0, lang_1.t)('form-creator.stage-selector-label'), stageTypes, setDefaultValues ? currentBracket.stage[0].type : undefined, 'base-input');\n}\n/**\n * removing all fields of the mask which are not the name and the stage selector\n *\n * @param config HTML element IDs to render this form to\n * @param stage Which type of stage are we building?\n * @param parent The parent DOM\n * @param submitCallback the callable to call when the data got created\n * @param submitBtnText Text of the submit button\n */\nfunction createMaskFields(config, stage, parent, submitCallback, submitBtnText) {\n  switch (stage) {\n    case 'round_robin':\n      // Teams amount\n      createInput(parent, 'number', config.html_group_id, (0, lang_1.t)('form-creator.group-label'), (0, lang_1.t)('form-creator.group-placeholder'), config.group_default_size.toString(), '1');\n      // Seed ordering\n      createSelect(parent, config.html_seed_order_id, (0, lang_1.t)('form-creator.seed-order-label'), roundRobinSeeds);\n      // Round robin mode\n      createSelect(parent, config.html_round_robin_mode_id, (0, lang_1.t)('form-creator.round-robin-mode-label'), roundRobinMode);\n      break;\n    case 'single_elimination':\n      // Seed ordering\n      createSelect(parent, config.html_seed_order_id, (0, lang_1.t)('form-creator.seed-order-label'), eliminationOrderings);\n      // Consolation Final\n      createInput(parent, 'checkbox', config.html_consolation_final_checkbox_id, (0, lang_1.t)('form-creator.consolation-final-label'));\n      break;\n    case 'double_elimination':\n      // Consolation Final\n      createInput(parent, 'checkbox', config.html_consolation_final_checkbox_id, (0, lang_1.t)('form-creator.consolation-final-label'));\n      // Skip first round\n      createInput(parent, 'checkbox', config.html_skip_first_round_checkbox_id, (0, lang_1.t)('form-creator.skip-first-round-label'));\n      // Grand final types\n      createSelect(parent, config.html_grand_final_type_id, (0, lang_1.t)('form-creator.grand-final-type-label'), grandFinalTypes);\n      // Seed orders\n      createTextarea(parent, config.html_double_elimination_seed_textarea_id, (0, lang_1.t)('form-creator.seed-order-label'), (0, lang_1.t)('form-creator.double-elimination-seed-order-placeholder'), 'natural');\n      break;\n    default:\n      throw new DOMException(`stage ${stage} seems to be not implemented yet.`);\n  }\n  const submitBtnWrapper = document.createElement('div');\n  const submitBtn = document.createElement('button');\n  submitBtn.innerText = submitBtnText || (0, lang_1.t)('form-creator.submit');\n  submitBtn.type = 'submit';\n  submitBtnWrapper.appendChild(submitBtn);\n  submitBtn.onclick = () => {\n    applyForm(config, stage, submitCallback);\n  };\n  parent.appendChild(submitBtnWrapper);\n}\n/**\n * Apply a form when click on submit\n *\n * @param config HTML element IDs to render this form to\n * @param stage Which type of stage are we building?\n * @param submitCallback the callable to call when the data got created\n */\nfunction applyForm(config, stage, submitCallback) {\n  var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k;\n  let payload;\n  let teamNamesValue;\n  switch (stage) {\n    case 'round_robin':\n      try {\n        validateRoundRobin(config);\n      } catch (e) {\n        alert(e.message);\n        return;\n      }\n      const roundRobinSettings = {\n        seedOrdering: [(_a = document.getElementById(config.html_seed_order_id).value) !== null && _a !== void 0 ? _a : 'groups.effort_balanced'],\n        roundRobinMode: (_b = document.getElementById(config.html_round_robin_mode_id).value) !== null && _b !== void 0 ? _b : 'simple',\n        groupCount: parseInt((_c = document.getElementById(config.html_group_id).value) !== null && _c !== void 0 ? _c : '0')\n      };\n      payload = {\n        name: (_d = document.getElementById(config.html_name_id).value) !== null && _d !== void 0 ? _d : '',\n        settings: roundRobinSettings,\n        tournamentId: 0,\n        type: stage\n      };\n      teamNamesValue = (_e = document.getElementById(config.html_team_names_input_id)) === null || _e === void 0 ? void 0 : _e.value;\n      if (teamNamesValue) payload.seeding = teamNamesValue.split(',');else {\n        const teamCount = parseInt(document.getElementById(config.html_team_count_input_id).value);\n        payload.seeding = Array.from({\n          length: teamCount\n        }).map((_, i) => `Team ${i + 1}`);\n        roundRobinSettings.size = brackets_manager_1.helpers.getNearestPowerOfTwo(teamCount);\n      }\n      break;\n    case 'single_elimination':\n      validateSingleElimination(config);\n      const singleEliminationSettings = {\n        seedOrdering: [(_f = document.getElementById(config.html_seed_order_id).value) !== null && _f !== void 0 ? _f : 'natural'],\n        consolationFinal: document.getElementById(config.html_consolation_final_checkbox_id).checked\n      };\n      payload = {\n        name: (_g = document.getElementById(config.html_name_id).value) !== null && _g !== void 0 ? _g : '',\n        settings: singleEliminationSettings,\n        tournamentId: 0,\n        type: stage\n      };\n      teamNamesValue = (_h = document.getElementById(config.html_team_names_input_id)) === null || _h === void 0 ? void 0 : _h.value;\n      if (teamNamesValue) payload.seeding = teamNamesValue.split(',');else {\n        const teamCount = parseInt(document.getElementById(config.html_team_count_input_id).value);\n        payload.seeding = Array.from({\n          length: teamCount\n        }).map((_, i) => `Team ${i + 1}`);\n        singleEliminationSettings.size = brackets_manager_1.helpers.getNearestPowerOfTwo(teamCount);\n      }\n      break;\n    case 'double_elimination':\n      validateDoubleElimination(config);\n      const rawSeedOrder = document.getElementById(config.html_double_elimination_seed_textarea_id).value.split(',');\n      const seedOrder = rawSeedOrder.map(order => order.trim());\n      const doubleEliminationSettings = {\n        seedOrdering: seedOrder,\n        consolationFinal: document.getElementById(config.html_consolation_final_checkbox_id).checked,\n        skipFirstRound: document.getElementById(config.html_skip_first_round_checkbox_id).checked,\n        grandFinal: document.getElementById(config.html_grand_final_type_id).value\n      };\n      payload = {\n        name: (_j = document.getElementById(config.html_name_id).value) !== null && _j !== void 0 ? _j : '',\n        settings: doubleEliminationSettings,\n        tournamentId: 0,\n        type: stage\n      };\n      teamNamesValue = (_k = document.getElementById(config.html_team_names_input_id)) === null || _k === void 0 ? void 0 : _k.value;\n      if (teamNamesValue) payload.seeding = teamNamesValue.split(',');else {\n        const teamCount = parseInt(document.getElementById(config.html_team_count_input_id).value);\n        payload.seeding = Array.from({\n          length: teamCount\n        }).map((_, i) => `Team ${i + 1}`);\n        doubleEliminationSettings.size = brackets_manager_1.helpers.getNearestPowerOfTwo(teamCount);\n      }\n      break;\n    default:\n      throw new DOMException(`stage ${stage} seems to be not implemented yet.`);\n  }\n  submitCallback(payload);\n}\n/**\n * validates everything for the double_elimination stage\n *\n * @param config FormConfig\n */\nfunction validateDoubleElimination(config) {\n  baseValidation(config);\n  const grandFinalType = document.getElementById(config.html_grand_final_type_id).value;\n  if (!grandFinalType || !grandFinalTypes.includes(grandFinalType)) throw new DOMException('grand_final_type must be one of: ' + grandFinalTypes.toString());\n  const orderings = document.getElementById(config.html_double_elimination_seed_textarea_id).value.split(',');\n  orderings.forEach(value => {\n    const ordering = value.trim();\n    if (!eliminationOrderings.includes(ordering)) throw new DOMException('elimination seed_ordering wrong found: ' + ordering + 'must be one of: ' + eliminationOrderings.toString());\n  });\n}\n/**\n * validates everything for a single_elimination stage\n *\n * @param config FormConfiguration\n */\nfunction validateSingleElimination(config) {\n  baseValidation(config);\n  const ordering = document.getElementById(config.html_seed_order_id).value;\n  if (!ordering || !eliminationOrderings.includes(ordering)) throw new DOMException('seed_ordering must be one of: ' + eliminationOrderings.toString());\n}\n/**\n * validates everything for a round_robin stage\n *\n * @param config FormConfiguration\n */\nfunction validateRoundRobin(config) {\n  baseValidation(config);\n  const groupAmount = parseInt(document.getElementById(config.html_group_id).value);\n  if (groupAmount <= 0) throw new DOMException('group_amount must be equal or bigger than 1');\n  const ordering = document.getElementById(config.html_seed_order_id).value;\n  if (!roundRobinSeeds.includes(ordering)) throw new DOMException('seed_ordering must be one of ' + roundRobinSeeds.toString());\n  const roundRobinMode = document.getElementById(config.html_round_robin_mode_id).value;\n  if (!roundRobinMode.includes(roundRobinMode)) throw new DOMException('round_robin_mode must be one of ' + roundRobinMode.toString());\n}\n/**\n * validates the field every specification has (name, teams and stage)\n *\n * @param config FormConfiguration\n */\nfunction baseValidation(config) {\n  var _a;\n  const name = document.getElementById(config.html_name_id).value;\n  if (!name || name === '') throw new DOMException('No name provided.');\n  const teamNamesValue = (_a = document.getElementById(config.html_team_names_input_id)) === null || _a === void 0 ? void 0 : _a.value;\n  const teams = teamNamesValue ? teamNamesValue.split(',') : [];\n  const teamCount = teams.length || parseInt(document.getElementById(config.html_team_count_input_id).value);\n  if (teamCount < 2) throw new DOMException('Invalid team amount provided.');\n  const stageType = document.getElementById(config.html_stage_type_selector_id).value;\n  if (!stageType && stageTypes.includes(stageType)) throw new DOMException('Invalid stage.');\n}\n/**\n * @param parent the parent DOM element to render to\n * @param textareaId the id of the textarea\n * @param labelText the text for the label of the textarea\n * @param textareaPlaceholder the placeholder for the textarea\n * @param textareaDefaultValue the default value for the textarea - if NULL or UNDEFINED this is not set\n * @param className Class name to give to the wrapper\n */\nfunction createTextarea(parent, textareaId, labelText, textareaPlaceholder, textareaDefaultValue, className) {\n  const wrapper = document.createElement('div');\n  const label = document.createElement('label');\n  label.setAttribute('for', textareaId);\n  label.innerText = labelText;\n  const textarea = document.createElement('textarea');\n  textarea.placeholder = textareaPlaceholder;\n  textarea.id = textareaId;\n  if (null !== textareaDefaultValue && undefined !== textareaDefaultValue) textarea.value = textareaDefaultValue;\n  if (className) wrapper.classList.add(className);\n  wrapper.appendChild(label);\n  wrapper.appendChild(textarea);\n  parent.appendChild(wrapper);\n}\n/**\n * @param parent parent to render the DOM to\n * @param inputType input type\n * @param inputId the id for the input\n * @param labelText the label text for the input\n * @param inputPlaceholder the placeholder for the input - if NULL or UNDEFINED this is not set\n * @param inputDefaultValue the default value for the input - if NULL or UNDEFINED this is not set\n * @param inputMinValue the min value for the input - if NULL or UNDEFINED this is not set\n * @param inputMinLengthValue the minLength value for the input - if NULL or UNDEFINED this is not set\n * @param className Class name to give to the wrapper\n */\nfunction createInput(parent, inputType, inputId, labelText, inputPlaceholder, inputDefaultValue, inputMinValue, inputMinLengthValue, className) {\n  const wrapper = document.createElement('div');\n  const label = document.createElement('label');\n  label.setAttribute('for', inputId);\n  label.innerText = labelText;\n  const input = document.createElement('input');\n  input.type = inputType;\n  input.id = inputId;\n  if (null !== inputPlaceholder && undefined !== inputPlaceholder) input.placeholder = inputPlaceholder;\n  if (null !== inputDefaultValue && undefined !== inputDefaultValue) input.value = inputDefaultValue;\n  if (null !== inputMinValue && undefined !== inputMinValue) input.min = inputMinValue;\n  if (null !== inputMinLengthValue && undefined !== inputMinLengthValue) input.minLength = inputMinLengthValue;\n  if (className) wrapper.classList.add(className);\n  wrapper.appendChild(label);\n  wrapper.appendChild(input);\n  parent.appendChild(wrapper);\n}\n/**\n * @param parent The wrapper to render the created DOM Elements to\n * @param selectId The ID of the generated select\n * @param labelText The text of the label for the generated select\n * @param options The options to display in select\n * @param selectedOption Which option should be selected by default\n * @param className Class name to give to the wrapper\n */\nfunction createSelect(parent, selectId, labelText, options, selectedOption, className) {\n  const wrapper = document.createElement('div');\n  const label = document.createElement('label');\n  label.setAttribute('for', selectId);\n  label.innerText = labelText;\n  const select = document.createElement('select');\n  select.id = selectId;\n  createOptions(select, options, selectedOption);\n  if (className) wrapper.classList.add(className);\n  wrapper.appendChild(label);\n  wrapper.appendChild(select);\n  parent.appendChild(wrapper);\n}\n/**\n * @param optionSwitch HTMLElement to add the options to\n * @param options string list of possible options\n * @param selectedOption Which option should be selected by default\n */\nfunction createOptions(optionSwitch, options, selectedOption) {\n  options.forEach(option => {\n    const optionElement = document.createElement('option');\n    optionElement.innerText = option;\n    if (option === selectedOption) optionElement.selected = true;\n    optionSwitch.appendChild(optionElement);\n  });\n}\nwindow.stageFormCreator = stageFormCreator;\nwindow.updateFormCreator = updateFormCreator;\n\n//# sourceURL=webpack://brackets-viewer/./src/form.ts?\n}");
...
/******/ 	var __webpack_exports__ = __webpack_require__("./src/form.ts");
/******/ 	
/******/ })()
;